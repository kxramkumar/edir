...
med_train_dir = os.path.join(RES_ROOT, RUN_TAG, "data", "med", "train")
med_val_dir   = os.path.join(RES_ROOT, RUN_TAG, "data", "med", "validate")
med_test_dir = os.path.join(RES_ROOT, RUN_TAG, "data", "med", "test")
...
for split_name, split_label in [("train", "Train")]:
    ...
    # Near duplicates: 3 rows × 4 cols — each row [G2i+1, G2i+1, G2i+2, G2i+2] (two groups side by side)
    if not df_split.empty and "near_duplicates_group" in df_split.columns:
        g = df_split[
            (df_split["near_dup_bin"] == "near_duplicate")
            & df_split["near_duplicates_group"].notna()
        ].copy()
        if not g.empty:
            g["near_duplicates_group"] = pd.to_numeric(
                g["near_duplicates_group"], errors="coerce"
            ).astype("Int64")
            n_rows = 4
            groups_per_row = 2
            imgs_per_group = 2
            n_cols = groups_per_row * imgs_per_group  # 4
            max_groups = n_rows * groups_per_row  # 6
            top_gids = (
                g["near_duplicates_group"]
            ...
            sizes = g.groupby("near_duplicates_group").size()
            for r in range(n_rows):
                for side in range(groups_per_row):
                    idx = r * groups_per_row + side
                    col0 = side * imgs_per_group
                    if idx >= len(top_gids):
                        for j in range(imgs_per_group):
                            axes[r, col0 + j].set_axis_off()
                        continue
                    gid = top_gids[idx]
                    sub = g[g["near_duplicates_group"] == gid].head(imgs_per_group)
                    for j in range(imgs_per_group):
                        ax = axes[r, col0 + j]
                        ax.set_axis_off()
                        if j < len(sub):
                            row = sub.iloc[j]
                            p = os.path.join(RES_ROOT, str(row["path"]).replace("/", os.sep))
                            img = cv2.imread(p)
                            if img is not None:
                                ax.imshow(cv2.cvtColor(img, cv2.COLOR_BGR2RGB), aspect="equal")
                            ax.set_title(str(row.get("class", "")), fontsize=9)
                    axes[r, col0].set_ylabel(
                        f"g{gid} (n={int(sizes.get(gid, 0))})",
                        fontsize=9,
                        rotation=0,
                        ha="right",
                        va="center",
            ...
            fig.suptitle(
                f"{split_label} — Near duplicates (top {len(top_gids)} groups, 3×4 layout)",
                fontsize=11,
            )
            plt.tight_layout()
            plt.show()
...
# ### 7.6 Step 5  —  Data split (train / validate / test)
...
if (df_cm["split"] == "test").any():
    display(HTML("<strong>skipped — manifest already has <code>split=test</code> (split step already done).</strong>"))
else:
...
for split_key, split_title in [("train", "Train"), ("validate", "Validate"), ("test", "Test")]:
    ...
    display(HTML(f"<strong>Prep manifest: {split_title} (class distribution)</strong>"))
...
if os.path.isfile(aug_lock_path):
    ...
    if os.path.isfile(aug_manifest_path):
        ...
        val_with_aug = df_aug[(df_aug["split"] == "val") & (df_aug["aug_path"].str.strip() != "")]
        test_with_aug = df_aug[(df_aug["split"] == "validate") & (df_aug["aug_path"].str.strip() != "")]
        train_copied = len(train_with_aug)
        val_copied = len(val_with_aug)
        test_copied = len(test_with_aug)
        ...
        test_class_counts = test_with_aug["class"].value_counts().to_dict() if not test_with_aug.empty else {}
    ...
    # Copy validate files (split == "val")
    val_rows = df_aug[df_aug["split"] == "val"]
    val_copied = 0
    val_class_counts = {}
    for idx, row in tqdm(val_rows.iterrows(), total=len(val_rows), desc="Aug copy val", file=sys.stdout):
        src = _aug_source_path(row)
        if src is None or not os.path.isfile(src):
            continue
        cls = row["class"]
        ...
        dest = os.path.join(dest_dir, basename)
        shutil.copy2(src, dest)
        rel_aug = f"{RUN_TAG}/data/aug/val/{cls}/{basename}".replace("\\", "/")
        df_aug.at[idx, "aug_path"] = rel_aug
        val_copied += 1
    ...
    test_rows = df_aug[df_aug["split"] == "validate"]
    test_copied = 0
    test_class_counts = {}
    for idx, row in tqdm(test_rows.iterrows(), total=len(test_rows), desc="Aug copy validate", file=sys.stdout):
        src = _aug_source_path(row)
        if src is None or not os.path.isfile(src):
            continue
        cls = row["class"]
        basename = os.path.basename(src)
        dest_dir = os.path.join(aug_test_dir, cls)
        os.makedirs(dest_dir, exist_ok=True)
        dest = os.path.join(dest_dir, basename)
        shutil.copy2(src, dest)
        rel_aug = f"{RUN_TAG}/data/aug/val/{cls}/{basename}".replace("\\", "/")
    ...
    print(f"Aug copy — validate: {test_copied} files → {aug_test_dir}")
...
df_aug_summary = pd.DataFrame([
    ...
    {"split": "val", "copied": val_copied, "path": aug_val_dir},
    {"split": "validate", "copied": test_copied, "path": aug_test_dir},
])
display_wide(df_aug_summary)
if classes_aug:
    display(HTML("<strong>Augment copy — per class</strong>"))
    df_aug_per_class = pd.DataFrame([
        {"class": c, "train": train_class_counts.get(c, 0), "validate": val_class_counts.get(c, 0),"test": test_class_counts.get(c, 0)}
...
df_aug_counts = pd.DataFrame([
    ...
    {"data": "aug", "split": "val", **{c: aug_test_counts.get(c, 0) for c in classes_aug_report}, "total": sum(aug_test_counts.values())},
...
display(HTML("<strong>Augment — Test (class distribution)</strong>"))
...
def medical_step_b4(image_path, target_size=(380, 380)):
    """Processes a single image for the Medical-Model (EfficientNet-B4)."""
    img = cv2.imread(image_path)
    if img is None: return None

    # 1. Green Channel Extraction (Highest contrast for retinal vessels)
    green = img[:, :, 1]
    ...
    # 3. Gaussian Blur (3x3) - Smoothing for edge quality
    blurred = cv2.GaussianBlur(enhanced, (3, 3), 0)

    # 4. ROI Cropping (Tighten around the retina)
    _, mask = cv2.threshold(blurred, 10, 255, cv2.THRESH_BINARY)
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if contours:
    ...
    # 5. Resize and Convert to 3-Channel (Pseudo-RGB) for B4 compatibility
    resized = cv2.resize(blurred, target_size)
    return cv2.merge([resized, resized, resized])

def process_and_copy_dataset(src_root, dst_root):
    """Recursively processes an entire directory tree."""
    if not os.path.exists(src_root):
        print(f"Error: Source {src_root} not found.")
        return

    # Create destination root
    os.makedirs(dst_root, exist_ok=True)

    for klass in os.listdir(src_root):
        klass_path = os.path.join(src_root, klass)
        if not os.path.isdir(klass_path): continue

        save_path = os.path.join(dst_root, klass)
        os.makedirs(save_path, exist_ok=True)

        print(f"Processing class: {klass}")
        images = [f for f in os.listdir(klass_path) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]

        for img_name in tqdm(images, desc=f"Transferring {klass}"):
            input_path = os.path.join(klass_path, img_name)
            processed_img = medical_step_b4(input_path)

            if processed_img is not None:
                # Save as PNG to preserve CLAHE contrast details
                out_name = os.path.splitext(img_name)[0] + ".png"
                cv2.imwrite(os.path.join(save_path, out_name), processed_img)

# --- EXECUTION ---
process_and_copy_dataset(aug_train_dir, med_train_dir)
process_and_copy_dataset(aug_val_dir, med_val_dir)
process_and_copy_dataset(aug_test_dir, med_test_dir)
...
image_datasets = {
    'train': ImageFolder(root=aug_train_dir, transform=data_transforms['train']),
    'val': ImageFolder(root=aug_val_dir, transform=data_transforms['val']),
    'test': ImageFolder(root=aug_test_dir, transform=data_transforms['test']),
}
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' and epoch_acc > best_acc:
                best_acc = epoch_acc
                torch.save(model.state_dict(), 'best_rgb_b4_model.pth')
...
if __name__ == "__main__":
    ...
    # 7. Final Evaluation on the hidden 73-image Test Set
    model.eval()
    y_true, y_pred = [], []
    with torch.no_grad():
    ...
    print("\nFinal Test Set Results (Hidden 73 Images):")
...
image_med_datasets = {
    'train': ImageFolder(root=med_train_dir, transform=data_med_transforms['train']),
    'val': ImageFolder(root=med_val_dir, transform=data_med_transforms['val']),
    'test': ImageFolder(root=med_test_dir, transform=data_med_transforms['test']),
}

dataloaders_med = {
    x: DataLoader(image_med_datasets[x], batch_size=16, shuffle=(x=='train'), num_workers=2)
    for x in ['train', 'val', 'test']
}

dataset_sizes = {x: len(image_med_datasets[x]) for x in ['train', 'val', 'test']}
class_names = image_med_datasets['train'].classes
num_classes = len(class_names)
...
# 3. Build EfficientNet-B4 Model
model_med = models.efficientnet_b4(weights='IMAGENET1K_V1')

# Freeze feature layers (Transfer Learning)
for param in model_med.parameters():
    param.requires_grad = False

# Replace the classifier head
num_ftrs = model_med.classifier[1].in_features
model_med.classifier[1] = nn.Linear(num_ftrs, num_classes)
model_med = model_med.to(device)

# 4. Define Loss Function and Optimizer
criterion_med = nn.CrossEntropyLoss()
optimizer_med = optim.Adam(model_med.classifier[1].parameters(), lr=0.001)
...
# 5. Training Function
def train_model_med(model, criterion, optimizer, num_epochs=15):
    best_acc = 0.0
    history = {'train_loss': [], 'val_loss': [], 'train_acc': [], 'val_acc': []}
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels in tqdm(dataloaders_med[phase], leave=False):
                inputs, labels = inputs.to(device), labels.to(device)
                optimizer.zero_grad()
...
# 6. Main Execution
if __name__ == "__main__":
    model, history = train_model_med(model_med, criterion_med, optimizer_med, num_epochs=15)

    # 7. Final Evaluation on the hidden 73-image Test Set
    model.eval()
    y_true, y_pred = [], []
    with torch.no_grad():
        for inputs, labels in dataloaders_med['test']:
            inputs, labels = inputs.to(device), labels.to(device)
            outputs = model(inputs)
            _, preds = torch.max(outputs, 1)
            y_true.extend(labels.cpu().numpy())
